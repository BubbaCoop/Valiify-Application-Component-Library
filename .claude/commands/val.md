...
description: >
  ...
  using the Short App component library. Usage: /val <figma-url|screenshot-path>
...
5. If val/registry/components.json is missing, OR older than the newest
   file in stories/components/, run
   node val/tools/generate-registry.mjs first (hand-added figmaNodeIds,
...
5. Regression check FIRST — before any QA dispatch, for each frame:
     ...
     node val/tools/regression-check.mjs <run-dir> --frame <state> \
