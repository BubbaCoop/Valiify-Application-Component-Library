...
run directory (val/runs/) or registry (val/registry/components.json).
...
token lessons, CSS traps) is node_modules/@valiify/val-core/skills/extract-methodology/SKILL.md.

## Gate 0 — Metadata sweep (you, one cheap call)

Run `get_metadata` on the component node from the URL (file
`PA5pr1Q8KLfbjTxdAbFm0V`). Record the variant matrix: every variant's node id,
