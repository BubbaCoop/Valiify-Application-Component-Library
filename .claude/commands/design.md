...
description: >
  ...
  SvelteKit + Svelte 5 (runes) page built only from Short App
...
@valiify/shortapp-ui and the surface methodology. When a stage reports BLOCKED,
...
1. Resolve the surface: `--surface <id>` if given; the only surface if one
   ...
   (`design-methodology/<file>`, from the table above) — if not,
   stop and say so.
2. Create `val/runs/<yyyy-mm-dd>-design-<slug>/` with `00-input/`
...
3. Write manifest.json:
   { "kind": "design", "runId",
     "library": "shortapp", "package": "@valiify/shortapp-ui",
     "surface": "<id>", "methodology": "design-methodology/<file>",
...
`node val/tools/design/class-audit.mjs 02-concept/concept.v1.html
--library . --methodology <methodology> --tailwind-from .
--package @valiify/shortapp-ui --class-prefix va --class-strict` yourself — it costs no model tokens). On
...
1. Validate it — this costs no model tokens:
   `node val/tools/design/feedback-check.mjs <run-dir> --out json`
