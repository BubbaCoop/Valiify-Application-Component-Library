...
design-language references — .claude/skills/valiify-shortapp-ui/SKILL.md,
src/themes/valiify.css (the generated tokens) and the Design Tokens + Quick Reference
...
Fonts via @fontsource/inter — the Short App type system is INTER-ONLY (all 24 text styles; there is no mono register in this design system).
Consume the library through 1. src/reset.css  2. dist/shortapp-ui.css — link them in that order, and
...
Icons come from src/icons/sprite.svg, inlined per the vite-starter pattern —
but inline ONLY the <symbol>s the page references:
  node val/tools/sprite-subset.mjs src/icons/sprite.svg --used-by 04-build/index.html --out 04-build/sprite.svg
...
4. Verifiable data (amounts, percentages, IDs) is set per
   03-requirements.md §4: tabular-nums (the Short App system is Inter-only; no mono register exists — if the Figma appears to show a mono face, flag it as a design question rather than importing a font the token set doesn't define).
...
run, once each:
  node val/tools/geometry-check.mjs <run-dir> --all
...
initial builds): node val/tools/geometry-check.mjs <run-dir> --all
...
node_modules/@valiify/val-core/skills/visual-verification/SKILL.md — consult it when a
