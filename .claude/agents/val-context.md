...
1. Purpose & audience — what this page is for; applicant-facing (the Short App application flow — fields, cards, actions) or portal/reviewer-facing (the Portal Specific set: StatusTracker, Action, UtilityButton). This selects which component families apply.
...
4. Data semantics — which fields are verifiable data (IDs, amounts,
   percentages) → tabular-nums (the Short App system is Inter-only; no mono register exists — if the Figma appears to show a mono face, flag it as a design question rather than importing a font the token set doesn't define); which are labels/UI → the
   token text styles.
