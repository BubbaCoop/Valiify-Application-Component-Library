...
2. For EVERY frame (state = its input.frames[] state; omit --frame for
   the primary):
     node val/tools/screenshot.mjs <run-dir> --frame <state>
     node val/tools/grid-diff.mjs <run-dir> --frame <state>
     node val/tools/classify-tiles.mjs <run-dir> --frame <state> --accepted 06-accuracy/accepted.json --crops
...
6. Classify EVERY non-pass tile into exactly one of three classes, and
   ...
   documented in node_modules/@valiify/val-core/skills/visual-verification/SKILL.md.
7. Confirm overlay.png was produced.
8. Audit your own adjudication, per frame, and report the result:
     node val/tools/classify-tiles.mjs <run-dir> --frame <state> --accepted 06-accuracy/accepted.json --verify
